...
parser.add_argument('-pde', nargs=1, type= str, required=True, help ='Input matrix file.')
...
def retrieve_iPDEinter(structure, pdeMat, contact_lst, max_dist):
    chain_lst = [x.id for x in structure[0]]
    seqlen = [len(x) for x in structure[0]]
    ipde_avg = []

    for ch1_idx in range(len(chain_lst)):
        ch1_sta = sum(seqlen[:ch1_idx])
        ch1_end = ch1_sta + seqlen[ch1_idx]
        ipde_col = []

        for contact_ch in contact_lst[ch1_idx]:
            ch2_idx = chain_lst.index(contact_ch)
            ch2_sta = sum(seqlen[:ch2_idx])
            ch2_end = ch2_sta + seqlen[ch2_idx]

            remain_pdeMatrix = pdeMat[ch1_sta:ch1_end, ch2_sta:ch2_end]
            ...
            for res1 in structure[0][chain_lst[ch1_idx]]:
                ...
                for res2 in structure[0][contact_ch]:
                    ...
                    if res1.has_id('CA') and res2.has_id('CA'):
                        if res1['CA'] - res2['CA'] <= max_dist:
                            ipde_col.append(remain_pdeMatrix[mat_x, mat_y])

        if not ipde_col:
            ipde_avg.append(0)
        else:
            ipde_avg.append(float(np.mean(ipde_col)))  # promedio directo de PDE
...
def main():
    ...
    for idx in range(len(chains)):
        ...
        IF_plddt, contact_lst = retrieve_IFplddt(structure, chains[idx], chain2_lst, args.dist)
        plddt_lst.append(IF_plddt)
        remain_contact_lst.append(contact_lst)

    ## retrieve interface PAE at chain-level
    with open(args.pae[0],'rb') as f:
        data_pae = np.load(f, allow_pickle=True)

    with open(args.pde[0],'rb') as f:
        data_pde = np.load(f, allow_pickle=True)

    avgif_pae = retrieve_IFPAEinter(structure, data_pae, remain_contact_lst, args.dist)
    avgif_pde = retrieve_iPDEinter(structure, data_pde, remain_contact_lst, args.dist)

    ## calculate print

    print('avg_ipde is:')
    for ch in range(len(chains)):
      print(chains[ch]+' '+str(avgif_pde[ch]))
    
    print('avg_ipae is:')
    for ch in range(len(chains)):
      print(chains[ch]+' '+str(avgif_pae[ch]))
